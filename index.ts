...
async function findArbitrage(
  ...
  const pools = await pg.query(
    `SELECT pool_address, token0, token1, type FROM active_pools
  ...
  for (const row of pools.rows) {
    ...
    if (amountOut > 0n) swapAB.push({ pool: row.pool_address, type: row.type, amountOut });
  }
      ...
      onLogs: async (logs: any[]) => {
        for (const log of logs) {
          ...
          if (factory.type === 'v2') {
            ...
            if (!slot0) continue;
            tvl = 1000; // rough estimate, enough to pass filter
          }
          if (tvl < 800) continue;
...
async function scanChain(chain: typeof CHAINS[0], rpcManager: RPCRotator, wsClient: PublicClient) {
  ...
  for (const pool of pools.rows) {
    ...
    const aToken0 = (reserveData0 as any).aTokenAddress;
    const aToken1 = (reserveData1 as any).aTokenAddress;
